...
% At the end of the script, creates multi-layer nested data structure that is additive for all participants 
    ...
    %Layer 2 = Phase (Baseline (2min), Adaptation (10min) , Deadaptation (10 min) - will have to run seperately for each phase since they are separate Vicon trials
    %Layer 3 = More affected or Less affected
    %Layer 4 = Joint/force/variable
    %Layer 5 = Stance or Swing
    %Layer 6 = X, Y or Z plane (only for force and joints)
...
% Table variable abreviations
    ...
    % EAd = Early Adapt- strides 11–30 (Roemmich and Bastian 2015 Journal of Neurophysiology)
    % MAd = Mid Adapt - middle 30 strides
    % LAd = Late Adapt - last 30 strides
    % EDe = Early Deadapt - strides 11–30
...
%% Calculating phi (degrees) and Phase Coordination Inedx (PCI) for each timepoint
...
%% Step Length Asymetry (total, positional contribution, timing contribution, and velocity contribution)
    ...
    %Created a copy of MA & LA heel strike matricies as lhs and rhs so it is not affected for other analyses
    ...
    Xs_SHS1 = zeros(1,NumStrikes(~FastLeg)-1); % Slow leg ankle-COM at slow leg heelstrke 1
    Xf_SHS1 = Xs_SHS1; % Fast leg ankle-COM at slow leg heelstrke 1
    Xs_SHS2 = Xs_SHS1; % Slow leg ankle-COM at slow leg heelstrke 2 (SHS1 shifted by one position)
    Xf_SHS2 = Xs_SHS1; % Fast leg ankle-COM at slow leg heelstrke 2 (SHS1 shifted by one position)
...
%% Propulsion and Braking Asymmetry - relative to body weight 
    % X and Y force data are flipped 
     % Initialize with Nans, Set WindowLength to the length of the first gait cycle - for later intperpolation
   ...
   for i = 1:(length(LA_HeelStrikes)-1)
       ...
       LA_XForce_BW(i,1:length(TempWindow)) = (TempWindow(1,:))/BodyWeight; % Propoulsion/braking forces
...
%% Rate of Adaptation and Magnitude of Adaptation (SLA Dblsupand PropAsym): Exponential fitting 
    ...
    for i = 1:length(Timepoints)
        ...
        elseif strcmp(Phase, 'Adaptation') 
            if strcmp(CurrentTimepoint, 'EAd')
                if strcmp(DataRate, 'Stride')
                Variable_EAd = Variable(11:31);
                elseif strcmp(DataRate, 'Step')
                Variable_EAd = Variable(21:61);
                end
        ...
        elseif strcmp(Phase, 'Deadaptation') 
            if strcmp(CurrentTimepoint, 'EDe')
                if strcmp(DataRate, 'Stride')
                Variable_EDe = Variable(11:31);
                elseif strcmp(DataRate, 'Step')
                Variable_EDe = Variable(21:61);
                end
